...
export function createOCRReview({getVehicle,open,close,mutate}){
 ...
 async function start(receipt,onSaved){
  ...
  open('사진에서 글자를 읽고 있어요',`<p class="detail-vehicle">${esc(v.name)} · 외부 전송 없음</p><p id="ocr-status" role="status">한국어 인식기를 준비하고 있어요.</p><p class="muted-note">사진 크기와 기기 성능에 따라 시간이 걸릴 수 있어요.</p><button id="ocr-cancel" class="secondary-button">인식 취소</button>`);
 ...
 function edit(d,raw,receipt,onSaved,v){
  ...
  $('#ocr-form').onsubmit=e=>{e.preventDefault();const int=id=>{const x=$(id).value.trim();return x===''?null:/^\d+$/.test(x)?Number(x):NaN;};
   ...
   open('이 내용으로 기록할까요?',`<p class="detail-vehicle">${esc(v.name)}</p><p>${esc(draft.date)} · ${fmt(draft.km)} km</p><p>방문 총비용: ${draft.cost===null?'미입력':fmt(draft.cost)+'원'}</p><ul>${selected.map(x=>`<li>${esc(x.name)} · ${{replace:'교환',inspect:'점검',clean:'청소',refill:'보충',adjust:'조정',lubricate:'윤활',other:'수리·기타'}[x.action]}</li>`).join('')}</ul><p class="demo-warning">예시 데이터에만 반영되며 새로고침하면 초기화됩니다.</p><button class="primary-button" id="ocr-save">확인한 기록 저장</button><button class="secondary-button" id="ocr-back">돌아가서 수정</button>`);

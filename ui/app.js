...
function render(){const v=vehicle();$('#vehicle-picker').innerHTML=`<span><span class="vehicle-name">${esc(v.name)}</span><span class="vehicle-meta">${esc(v.meta)}</span></span>${icon('down')}`;
 ...
 if(APP_MODE&&!visibleVehicles().length){$('#main').innerHTML='<div class="empty"><h2>내 차량을 등록해 주세요</h2><button class="primary-button" data-add-vehicle>차량 등록</button><button class="secondary-button" data-trash>삭제한 차량 복구</button><button class="secondary-button" data-backup>백업 불러오기</button></div>';return;}
